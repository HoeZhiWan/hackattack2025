...
export default function NetworkTrafficAnalysisPage() {
  ...
  // Fetch Suricata status and alerts on mount and when actions are performed
  ...
  useEffect(() => {
    ...
    // const interval = setInterval(refreshStatusAndAlerts, 5000);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setTimeout(refreshStatusAndAlerts, 1000); // Give Suricata a moment to start
    } catch (err) {
  ...
  const handleKill = async () => {
    ...
    try {
      ...
      setTimeout(refreshStatusAndAlerts, 1000); // Give Suricata a moment to stop
    } catch (err) {
